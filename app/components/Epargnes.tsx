...
export default function Epargnes({ data, setData }: any) {
  ...
  function openSaisie(enveloppe: string) {
    ...
    setSaisieForm({ versement: existing.versement || '', retrait: existing.retrait || '', valorisation: existing.valorisation || '' })
    setShowSaisie(true)
  }
  ...
  async function saveConfig() {
    const updated = { ...data.epargneConfig, baptiste: configForm }
  ...
  return (
    <div>
      ...
      {/* HEADER */}
      <div style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', padding: '16px 20px 12px', gap: 8 }}>
        ...
        {totalObjectif > 0 && (
          <div style={{ padding: '12px 18px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: 'var(--text-sub)', marginBottom: 6 }}>
              ...
              <span>{totalObjectif.toLocaleString('fr-FR')} € objectif</span>
            </div>
      ...
      <div style={{ marginBottom: 12 }}>
        <div style={{ padding: '8px 20px 6px', fontSize: 12, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 0.8, color: 'var(--text-sub)' }}>Enveloppes</div>
            ...
            <svg width={120} height={120} viewBox="0 0 120 120">
              {(() => {
                ...
                return enveloppes.map((env, i) => {
                  ...
                  const el = (
                    <circle key={env} cx={cx} cy={cy} r={r} fill="none"
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', zIndex: 90, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}
          ...
          <div onClick={e => e.stopPropagation()} style={{ background: 'white', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 430, padding: '20px 20px 40px' }}>
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              <input value={newEnveloppe.nom} onChange={e => setNewEnveloppe(p => ({ ...p, nom: e.target.value }))} placeholder="Ex: SCPI"
                style={{ width: '100%', background: '#f2f2f2', border: 'none', borderRadius: 10, padding: '12px 14px', fontSize: 15, fontFamily: 'DM Sans, sans-serif', outline: 'none' }} />
            </div>
            <div style={{ marginBottom: 14 }}>
              <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-sub)', marginBottom: 6 }}>Objectif fin 2026 (€)</div>
              <input type="number" value={newEnveloppe.objectif} onChange={e => setNewEnveloppe(p => ({ ...p, objectif: e.target.value }))} placeholder="0"
                style={{ width: '100%', background: '#f2f2f2', border: 'none', borderRadius: 10, padding: '12px 14px', fontSize: 15, fontFamily: 'DM Sans, sans-serif', outline: 'none' }} />
            </div>
            <div style={{ display: 'flex', gap: 10, marginTop: 8 }}>
